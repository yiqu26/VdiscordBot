...
class AutoMediaBot(discord.Client):
    ...
    async def loading_animation(self, message, sender):
        loading_stages = ["⏳ 載入中.", "⏳ 載入中..", "⏳ 載入中..."]
        for i in range(6):
            stage = loading_stages[i % len(loading_stages)]
            try:
                await message.edit(content=f"{stage} @{sender} 提供的影片處理中")
            except:
                pass
        ...
        if "instagram.com/reel/" in message.content or "instagram.com/p/" in message.content:
            for word in message.content.split():
                if "instagram.com/reel/" in word or "instagram.com/p/" in word:
                    ...
                    loading_msg = await message.channel.send("⏳ 嘗試連結中...")
                    task = asyncio.create_task(self.loading_animation(loading_msg, sender))
